def find_median_five(L):
  ...
  # 5개의 그룹으로 나누었을 때 이곳에서의 중앙값 중에서 중앙값을 구해야 함
  
  length = len(L) # 리스트 길이
  median = int(length / 2) # 리스트 길이의 중간에 위치하는 값이 중앙값일 것
  
  # 예를 들어 정렬된 7개의 값이 있으면 7/2 = 3해서 L[3]에 중앙값이 있는 것
  # 이 경우 length = 7, median = 3 
  
  if length % 2 == 1: # 주어진 리스트의 길이가 홀수일 경우, length 7 % 2 == 1
    return L[median] # 중앙값, L[3]에 위치하는 값이 정렬된 기준 중앙값
  
  else: # 홀수가 아닐 경우, length 8일 경우
    # 리스트 안에 8개의 요소가 있을 경우 중앙값은 L[3], L[4] 두개의 사이 값
    # median = 8 / 2 = 4
